...
var get_index_nav = function(market){
    ...
    pg.connect(conString_daily, function(err, client) {
        ...
        client.query(sqlstr, [market_index[market], '2012-05-25'], function(err, result) {
            if (err) {
                def.reject(err);
            }
            else {
...
var get_market_alpha_sum = function(market, p_or_n){
    ...
    pg.connect(conString_daily, function(err, client) {
        ...
        sqlstr = "SELECT to_char(da,'YYYY-MM-DD') as da, " + p_or_n +" as alpha from public.market_alpha_sum where market = '" + market + "' and da>=" + "'2012-05-25' and which_pool = 'big_pool' order by da asc;";
...
app.get('/chart/monitor/:market', function(req, res){
    ...
    var p2 = get_model_nav(market,'claude');
    ...
    .then(function(data){
        ...
        res.render('nav_monitor.jade', render_data);
    });

});

app.get('/monitor', function(req, res){
    res.render('nav_monitor.jade');
});
